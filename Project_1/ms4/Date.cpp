	...
	bool Date::validate()
	{
		...
		if ((year >= sdds_testYear) && (year <= MAX_YEAR_VALUE))
		{
			if ((month > 0) && (month < 13))
			{
				...
				{
					d_state.operator=("Invalid day in date");
					d_state.operator=(3);
				}
			}
			else
			{
				d_state.operator=("Invalid month in date");
				d_state.operator=(2);
			}
		}
		else
		{
			d_state.operator=("Invalid year in date");
			d_state.operator=(1);
		}
		return flag;
	}
...
Create an accessor method called state that returns a
constant reference to the State of the Date.*/
	}
...
If the Date object is casted to a boolean,
the state of the date object is returned.*/
	}
